...
func (bot *telegramBot) call(ctx context.Context, config telegramRuntimeConfig, method string, payload any, result any) error {
	...
	client, err := bot.httpClient(ctx, config.Proxy, config.ViaInterface)
	if err != nil {
		return err
	}
...
func (bot *telegramBot) httpClient(ctx context.Context, proxy, viaInterface string) (*http.Client, error) {
	proxy = strings.TrimSpace(proxy)
	viaInterface = strings.TrimSpace(viaInterface)
	bot.clientMu.Lock()
	defer bot.clientMu.Unlock()
	if bot.client != nil && bot.clientProxy == proxy && bot.clientInterface == viaInterface {
		return bot.client, nil
	}
	client, err := persistentTelegramHTTPClient(ctx, 10*time.Second, proxy, viaInterface)
	if err != nil {
		return nil, err
	}
	if bot.client != nil {
	...
	bot.clientInterface = viaInterface
	bot.client = client
	return client, nil
}
